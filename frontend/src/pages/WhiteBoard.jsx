...
const Whiteboard = () => {
    ...
    useEffect(() => {
        ...
        canvas.width = window.innerWidth * 0.8; // 80% width for canvas
    ...
    const draw = (e) => {
        ...
        if (isErasing) {
            ...
            socket.emit("draw", { x, y, prevX: prevPos.x, prevY: prevPos.y, color, size });
            ctx.strokeStyle = color;
            ctx.lineWidth = size;
            ctx.beginPath();
    ...
    return (
        <div style={{ display: "flex" }}>
            <canvas
                ...
                style={{ border: "1px solid black", cursor: "crosshair", flex: "1" }}
            />

            <div style={{ width: "20%", padding: "10px", display: "flex", flexDirection: "column", gap: "10px", borderLeft: "2px solid #ccc", background: "#f8f9fa" }}>
                ...
                <button onClick={clearWhiteboard} style={{ background: "#ffcccb" }}>Clear Whiteboard</button>
            </div>
        </div>
    );
};
